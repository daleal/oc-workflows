...
export type StructuredPromptInput<Schema extends z.ZodType> = Omit<
  PromptInput,
  'format' | 'noReply'
> & {
  schema: Schema;
...
const tokenCount = (message: AssistantMessage) => {
  return (
...
const activity = (part: ToolPart) => {
  const detail = value(part.state.input ?? {});
  return detail ? `${part.tool} ${detail}` : part.tool;
};

const observe = (runtime: WorkflowRuntime, name: string) => {
  ...
  const snapshot = (): WorkflowProgress => {
    ...
    const tasks = [...sessions.values()]
      ...
      .sort((first, second) => first.info.time.created - second.info.time.created)
      .map((session) => {
        const running = [...session.tools.values()].findLast(
...
export const promptStructured = async <Schema extends z.ZodType>(
...
export const text = (parts: Array<{ type: string; text?: string }>) => {
  return parts
    .filter(
