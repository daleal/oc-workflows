...
const parse = (value: unknown): WorkflowProgress | undefined => {
  ...
  const tasks = progress.tasks.flatMap((value) => {
    ...
    if (
      ...
      !['pending', 'running', 'completed', 'error'].includes(task.status)
    )
      return [];
...
const parseState = (value: unknown): WorkflowState | undefined => {
...
const detail = (task: WorkflowProgress['tasks'][number]) => {
  return [
...
const marker = (status: TaskStatus) => {
...
const currentSession = (api: TuiPluginApi) => {
...
const View = (props: { api: TuiPluginApi; runs: () => Map<string, WorkflowState> }) => {
...
const tui: TuiPlugin = async (api) => {
  ...
  const refresh = async () => {
    ...
    try {
      ...
      setRuns(
        new Map(states.flatMap((state) => (state ? [[state.sessionID, state] as const] : []))),
      );
    } finally {
      reading = false;
    }
